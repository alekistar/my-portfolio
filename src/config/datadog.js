let datadogRumInstance = null;
let datadogLogsInstance = null;
let datadogLoadPromise = null;

const loadDatadog = async () => {
  if (!datadogLoadPromise) {
    datadogLoadPromise = Promise.all([
      import('@datadog/browser-rum'),
      import('@datadog/browser-logs'),
    ]).then(([rumModule, logsModule]) => {
      datadogRumInstance = rumModule.datadogRum;
      datadogLogsInstance = logsModule.datadogLogs;
      return {
        datadogRum: datadogRumInstance,
        datadogLogs: datadogLogsInstance,
      };
    });
  }

  return datadogLoadPromise;
};
...
export const trackEvent = (eventName, eventData = {}) => {
  if (process.env.NODE_ENV === 'production') {
    if (datadogRumInstance) {
      datadogRumInstance.addAction(eventName, eventData);
      return;
    }
